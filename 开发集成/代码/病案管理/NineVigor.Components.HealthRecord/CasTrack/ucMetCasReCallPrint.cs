...
{
    public partial class ucMetCasReCallPrint : NineVigor.FrameWork.WinForms.Controls.ucBaseControl
    {
        public ucMetCasReCallPrint()
        {
            InitializeComponent();
        }

        #region 变量
        string strSQL = @"   select cg.dept_name 科室名称,
       cg.patient_no 病案号,
       ...
       to_Char(cg.out_Date,'yyyy-MM-dd') 出院日期,cg.inpatient_no 住院流水号
 ...
 where cg.checkdate >= to_date('{0}', 'yyyy-MM-dd HH24:mi:ss')
   ...
   and (cg.dept_Code='{2}' or 'ALL'='{2}') and (cg.opercode='{3}' or 'ALL'='{3}') {4} 
   ...
   '{10}')";
       ...
       // NorthChina.HealthRecord.Common.ConstVariable constVar = new NorthChina.HealthRecord.Common.ConstVariable();
        ...
        private void OnPrintA4Record()
        {
            ...
            {
                for (int i = 0; i < this.nineSpread2_Sheet1.RowCount; i++)
                {
                    string inSeqNo=dataPre+"-"+seqNoStart.ToString().PadLeft(5,'0');
                    //住院号，
                    string exeExcSql = string.Format(insertSql,
                        this.nineSpread2_Sheet1.Cells[i, 8].Text,
                        this.nineSpread2_Sheet1.Cells[i, 1].Text,
                        this.nineSpread2_Sheet1.Cells[i, 2].Text,
                        inSeqNo,
                    "","","","",
                    dataPre,
                    "",
                    ...
                    if (result < 0)
                    {
                        ...
                        MessageBox.Show("病案号:"+this.nineSpread2_Sheet1.Cells[i, 1].Text+" 生成打印货架号出错。");
                        return ;
                    }
                    NineVigor.FrameWork.Management.PublicTrans.Commit();

                    ucMetCasReCallPrintPanel2 pp = new ucMetCasReCallPrintPanel2();
                    string[] arrs = new string[] { this.nineSpread2_Sheet1.Cells[i, 1].Text, 
                        this.nineSpread2_Sheet1.Cells[i, 3].Text,
                        this.nineSpread2_Sheet1.Cells[i, 0].Text, 
                        this.nineSpread2_Sheet1.Cells[i, 8].Text,
                    this.nineSpread2_Sheet1.Cells[i, 7].Text,
                    inSeqNo,
                    seqNoStart.ToString()
                    };
        ...
        private string GetSeqNo(ref int seqNo)
        {
            ...
            string exeSql = @"select nvl(max(g.seqno),1) 序号 from Met_Cas_Firstpage_print g
