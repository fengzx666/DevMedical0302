...
namespace NineVigor.Components.HealthRecord.CasTrack
{
    ...
    /// 病案回收信息查询
    /// </summary>
    public partial class ucPageCallBackCheckByDayReport : NineVigor.FrameWork.WinForms.Controls.ucBaseControl
    {
        public ucPageCallBackCheckByDayReport()
        {
            InitializeComponent();
        }
        ...
        #region 变量
        #region
        string strSQL = @"select BB.DEPT_CODE,nvl((select dept_name
    from com_department
   where dept_code =BB.DEPT_CODE and rownum=1),'合计') 部门名称,
       sum(1) 出院总人数,
       sum(case
  ...
  from (select AA.*,
               ...
               mcf.opercode 回收人
          from (select *
                  from met_Cas_dayreport_detail mcd
                 where mcd.stat_type='O' and mcd.valid_state='0' and (DEPT_CODE='{2}' or 'ALL'='{2}')
                   and trunc(mcd.stat_date) >=
                       to_date('{0}', 'yyyy-MM-dd')
                   and trunc(mcd.stat_date) <=
                       to_date('{1}', 'yyyy-MM-dd')) AA
 ...
 group by rollup(BB.dept_code)";
       ...
       BB.dept_name,
       BB.PATIENT_NO,
       to_char(BB.In_Times) 入院次数,
       BB.Name,
       decode(BB.Sex_Code, 'F', '女', 'M', '男', '其他') 性别,
       to_Char(BB.In_Date, 'yyyy-MM-dd') 入院日期,
       to_char(BB.stat_date, 'yyyy-MM-dd') 出院日期,
       BB.HOUSE_DOC_NAME 住院医师,
       (BB.tot_cost + BB.balance_cost) 费用合计
  from (select AA.*, mcf.checkdate 回收时间, mcf.opercode 回收人
          from (select fii.*, cg.stat_date
                  from (select *
                          from met_cas_dayreport_detail mcd
                         where mcd.stat_type = 'O'
                           and mcd.valid_state = '0'
                           and (DEPT_CODE = '{2}' or 'ALL' = '{2}')
                           and trunc(mcd.stat_date) >=
                               to_date('{0}', 'yyyy-MM-dd')
                           and trunc(mcd.stat_date) <=
                               to_date('{1}', 'yyyy-MM-dd')) CG
                  left join fin_ipr_inmaininfo fii
                    on fii.inpatient_no = CG.inpatient_no) AA
          left join met_cas_firstpagecheck mcf
            on AA.inpatient_no = mcf.inpatient_no) BB
 where BB.回收人 is null
 order by BB.dept_name, BB.stat_date";
        #endregion
 ...
 order by BB.回收时间";
        ...
        protected override void OnLoad(EventArgs e)
        {
          ...
          //  al = interManager.GetDeptment(Neusoft.HISFC.Models.Base.EnumDepartmentType.I);
            al.Insert(0, new NineVigor.FrameWork.Models.NineObject("ALL", "全部", ""));
            cmbDept.AddItems(al);
        ...
        {
            this.nineSpread1_Sheet1.Columns[2].Locked = true;
            this.nineSpread1_Sheet1.Columns[3].Locked = true;
            this.nineSpread1_Sheet1.Columns[4].Locked = true;
            

            this.nineSpread2_Sheet1.Columns[0].Locked = true;
            this.nineSpread2_Sheet1.Columns[1].Locked = true;
            this.nineSpread2_Sheet1.Columns[2].Locked = true;
            this.nineSpread2_Sheet1.Columns[3].Locked = true;
            this.nineSpread2_Sheet1.Columns[4].Locked = true;
            this.nineSpread2_Sheet1.Columns[5].Locked = true;
            

            for (int i = 0; i < nineSpread1_Sheet1.Rows.Count; i++)
            {
                //总人数，和已经回收人数比较
                int unCol = NineVigor.FrameWork.Function.NConvert.ToInt32(this.nineSpread1_Sheet1.Cells[i, 2].Text);
                int havCol = NineVigor.FrameWork.Function.NConvert.ToInt32(this.nineSpread1_Sheet1.Cells[i, 4].Text);

                if (!"合计".Equals(this.nineSpread1_Sheet1.Cells[i, 1].Text))
                {
                    if (unCol == havCol)
                    {
                        nineSpread1_Sheet1.Rows[i].BackColor = Color.White;
                    }
                    else
                    {
                        nineSpread1_Sheet1.Rows[i].BackColor = Color.Red;
                    }
                }
                else
                {
        ...
        protected int Query()
        {
            ...
            tempSQL = string.Format(strSQL, dtBegin.ToString("yyyy-MM-dd"), dtEnd.ToString("yyyy-MM-dd"), strDept);
            try
            {
            ...
            try
            {
                ...
                #region
                this.nineSpread1_Sheet1.ColumnHeader.Cells.Get(2, 0).Value = "查询时间：" + dtpBeginDate.Value.ToShortDateString() +
                                                                            " 至 " + dtpEndDate.Value.ToShortDateString() +
                                                                            "    科室：" + strDeptNM +
                                                                            "    共" + dt.Rows.Count.ToString() + "条记录";

                nineSpread1_Sheet1.RowCount = 0;
                nineSpread1_Sheet1.DataSource = dt.DefaultView;
            ...
            {
                MessageBox.Show("错误1:" + ex.StackTrace + "  错误2:" + ex.Message + "错误3:" + ds.Tables.Count);
            }
        ...
        {
            switch (this.tabControl1.SelectedTabPageIndex)
            {
                case 0:
                ...
                case 1:
                    ...
                    break;
                case 2:
                    if (this.nineSpread3_Sheet1.RowCount > 0)
                    {
                        FarPoint.Win.Spread.PrintInfo pi = new FarPoint.Win.Spread.PrintInfo();
                        pi.Header = "/r /fz\"12\" 第 /p 页 共 /pc 页";// "/l第/p页共/pc页";
                        pi.ShowBorder = false;
                        ...
                        float len = 0f;
                        for (int cidx = 0; cidx < this.nineSpread3_Sheet1.Columns.Count; cidx++)
                        {
                            len += this.nineSpread3_Sheet1.Columns[cidx].Width;
                        }
                        ...
                        pi.UseMax = false;
                        this.nineSpread3_Sheet1.PrintInfo = pi;

                        this.nineSpread3.PrintSheet(this.nineSpread3.ActiveSheetIndex);
                    }
                    else
                    {
                        MessageBox.Show("没有要打印的数据");
        ...
        public override int Export(object sender, object neuObject)
        {
            ...
            this.nineSpread1_Sheet1.SaveHtml(dd.FileName);
            return 1;
        }
        ...
        private void neuSpread1_CellDoubleClick(object sender, FarPoint.Win.Spread.CellClickEventArgs e)
        {
            ...
            strSubNM = this.nineSpread1_Sheet1.Cells[e.Row, 0].Text;
            strDeptNM = this.nineSpread1_Sheet1.Cells[e.Row, 1].Text;


            if (String.IsNullOrEmpty(strSubNM))
            {
                strSubNM = "ALL";
                strDeptNM = "全部";
            }
            ...
            tempSQL = string.Format(strSQL2, dtBegin.ToString("yyyy-MM-dd"), dtEnd.ToString("yyyy-MM-dd"), strSubNM);
            string exeSql = string.Format(strSQL3, dtBegin.ToString("yyyy-MM-dd"), dtEnd.ToString("yyyy-MM-dd"), strSubNM);
            try
            {
            ...
            catch (Exception ex)
            {
                strErr = "查询数据失败！" + ex.Message;
                MessageBox.Show(strErr);
                nineSpread2_Sheet1.RowCount = 0;
                return;
            }

            if (ds2 != null && ds2.Tables.Count > 0 && ds2.Tables[0] != null)
            {
                this.nineSpread3_Sheet1.ColumnHeader.Cells.Get(1, 0).Value = "病案已回收 查询时间：" + dtpBeginDate.Value.ToShortDateString() +
                                                                           " 至 " + dtpEndDate.Value.ToShortDateString() +
                                                                           "    科室：" + strDeptNM +
                                                                           "    共" + ds2.Tables[0].Rows.Count.ToString() + "条记录";

                nineSpread3_Sheet1.RowCount = 0;

                nineSpread3_Sheet1.DataSource = ds2.Tables[0].DefaultView;
            }

            if (ds1 != null && ds1.Tables.Count > 0 && ds1.Tables[0] != null)
            {
                dt = ds1.Tables[0];
                this.nineSpread2_Sheet1.ColumnHeader.Cells.Get(1, 0).Value = "病案未回收 查询时间：" + dtpBeginDate.Value.ToShortDateString() +
                                                                           " 至 " + dtpEndDate.Value.ToShortDateString() +
                                                                           "    科室：" + strDeptNM +
                                                                           "    共" + dt.Rows.Count.ToString() + "条记录";
                ...
                nineSpread2_Sheet1.DataSource = dt.DefaultView;
            }
